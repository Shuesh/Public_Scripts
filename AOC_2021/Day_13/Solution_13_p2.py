...
def main():
    ...
    for fold in fold_list:
        if fold[0] == 'x':
            coordinate_map = Fold_Horizontal(coordinate_map, fold[1])
        elif fold[0] == 'y':
            coordinate_map = Fold_Vertical(coordinate_map, fold[1])
    
    letter_list = [None for _ in range(len(coordinate_map))]
    for row_index,row in enumerate(coordinate_map):
        letter_list[row_index] = ''.join(row)
